...
public class Gravicubes extends PThread {
	...
	public Gravicubes(PApplet p, int num_cubes) {
		...
		colour = p.color(p.random(0, 255), p.random(0, 255), p.random(0, 255), 125);
	}
	
	public Gravicubes(PApplet p) {
		super(p);
		cubes = new ArrayList<Cube>();
		NUM_CUBES = 1000;
		MIN_ROT = PI / 20;
	...
	@Override
	public void draw() {
		cubes.forEach(cube -> cube.display());
	}
	...
	class Cube {
		...
		Cube(PApplet p, PGraphics pg) {
			...
			this.colour = p.color(p.random(0, 255), p.random(0, 255), p.random(0, 255), 100);
